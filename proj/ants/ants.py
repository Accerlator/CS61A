...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        dist = 0
        while temp != beehive:
            if self.min_range <= dist <= self.max_range and len (temp.bees) != 0:
                return rANTdom_else_none(temp.bees)
            temp = temp.entrance
            dist = dist + 1
        return None
...
class ShortThrower(ThrowerAnt):
    ...
    #     temp = self
    #     while True:
    #         if times > self.max_range:
    #             return None
    #         if len(temp.place.bees) != 0:
    #             return  ThrowerAnt.nearest_bee(temp,beehive)
    #         temp.place = temp.place.entrance
    #         times = times + 1
    # END Problem 4
    ##上面是能跑的错误实现(不是根据题目意思来的)
...
class LongThrower(ThrowerAnt):
    ...
    #     times = 0
    #     temp = self
    #     while times <= self.min_range:
    #         if temp.place.entrance == None:
    #             return None
    #         temp.place = temp.place.entrance
    #         times = times + 1
    #     return ThrowerAnt.nearest_bee(temp,beehive)
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        while now_loc != None:
            if now_loc.ant != None:
                if now_loc.ant.is_doubled == False:
                    now_loc.ant.is_doubled = True
                    now_loc.ant.damage = now_loc.ant.damage * 2
                # else:
                #     now_loc.ant.damage = now_loc.ant.damage / 2
            now_loc = now_loc.exit
        # END Problem EC

...
def balanced(m):
    ...
    # if length(left(m)) * end(left(m)) == length(right(m)) * end(right(m)):
    #     return True
...
def totals_tree(m):
    ...
    if is_planet(m):
        return tree(size(m))
    return tree(label(totals_tree(end(left(m)))) + label(totals_tree(end(right(m)))),
...
def replace_leaf(t, find_value, replace_value):
    ...
    # return replace_leaf(branches(t),find_value, replace_value)
...
def preorder(t):
    ...
    #     return helper(i) for i in branches(t)
    res = []
    def helper(temp):
        nonlocal res
        if temp is not None:
            res.append(label(temp))
        for i in branches(temp):
            helper(i)
    helper(t)
    return res
...
def has_path(t, word):
    ...
    def helper(temp,cur):
        ...
        if cur not in word:
            return False
        return any([helper(i,cur + label(i)) for i in branches(temp)])
...
def quadratic(x, a, b, c):
    ...
    if  lower_bound(x) > (-1) * b / (2 * a) or upper_bound(x) < (-1) * b / (2 * a):
        return interval(a * lower_bound(x) * lower_bound(x) + b * lower_bound(x) + c
        ,a * upper_bound(x) * upper_bound(x) + b * upper_bound(x) + c)
    ##上面是区间不包括极值点的情况

    if a < 0:
        return interval(min(a * lower_bound(x) * lower_bound(x) + b * lower_bound(x) + c
        ,a * upper_bound(x) * upper_bound(x) + b * upper_bound(x) + c),(4 * a * c - b * b) / (4 * a))

    if a > 0:
        return interval((4 * a * c - b * b) / (4 * a)
        ,max(a * lower_bound(x) * lower_bound(x) + b * lower_bound(x) + c))
